...
class CustomArticleSentencesDataset(Dataset):
    def __init__(self, article_df, feature_label, tokenizer, max_source_len, device, target_type="mean_score", comment_df=None):
        ...
        self.article_ids = article_df["article_id"].values
...
class CustomModelForArticleEmbedding(nn.Module):
    def __init__(self, text_embedder_model_or_hf_id, device):
        ...
        if isinstance(text_embedder_model_or_hf_id, nn.Module):
            ...
            if "roberta-base" in text_embedder_model_or_hf_id:
    ...
    def forward(self, input_ids, attention_mask,targets=None, return_attention_weights=False,**kwargs):
        ...
        #####
        np.savetxt(os.path.join("./output_and_results/", "raw_tokenized_input.csv"),input_ids.cpu().detach().numpy() , delimiter=";")
        text_embed_cosine_sim_mat = cosine_similarity(input_ids.cpu().detach().numpy(), input_ids.cpu().detach().numpy())
  
        np.savetxt(os.path.join("./output_and_results/", "raw_tokenized_input_cosine_sim_mat.csv"), text_embed_cosine_sim_mat , delimiter=";")
        ...
        text_embeddings = cls_pooling(text_embeddings, attention_mask)
...
def load_embedder_model(pretrain_text_embedder, subtexts_aggregation_strategy, base_model_hf_id , train_dataset, art_df, cmt_df,device):
    ...
    if pretrain_text_embedder in ["SimCE", "TSDAE", "CosSim", "SimCE-Head-Body", "SimCE-Head-Comment"]:
        ...
        #base_model_hf_id = 'xlnet-large-cased'
        base_model_hf_id = 'xlnet-large-cased'
        ...
        if  pretrain_text_embedder == "TSDAE":
            model_output_dir = "./models/{}xlnet-TSDAE-sentence-embedder/".format(addendum_label) #"./models/distilroberta-SimCE-sentence-embedder/"
        elif pretrain_text_embedder == "SimCE":
            model_output_dir = "./models/{}xlnet-SimCE-sentence-embedder/".format(addendum_label)
        elif pretrain_text_embedder == "CosSim":
            model_output_dir = "./models/{}xlnet-CosSim-sentence-embedder/".format(addendum_label)
        elif pretrain_text_embedder == "SimCE-Head-Body":
            model_output_dir = "./models/{}xlnet-SimCE-Head-Body-sentence-embedder/".format(addendum_label)
        elif pretrain_text_embedder == "SimCE-Head-Comment":
            model_output_dir = "./models/{}xlnet-SimCE-Head-Comment-sentence-embedder/".format(addendum_label)
        ...
        try:
            ...
            word_embedding_model = sentence_models.Transformer(base_model_hf_id, max_seq_length=512)
            pooling_model = sentence_models.Pooling(word_embedding_model.get_word_embedding_dimension(), pooling_mode = "cls", pooling_mode_cls_token = True)
            text_sentence_embedder = SentenceTransformer(modules=[word_embedding_model, pooling_model], device=device) #'all-mpnet-base-v2')
            ...
            if pretrain_text_embedder == "SimCE":
                ...
                #train_examples = list(np.random.choice(train_examples, size=600, replace=False))
...
def main(model_output_dir, data_dirpath, HF_MODEL_ID, subtexts_aggregation_strategy, pretrain_text_embedder, target_type, deepspeed):
    ...
    tokenizer = RobertaTokenizer.from_pretrained(HF_MODEL_ID) if "roberta-base" in HF_MODEL_ID else AutoTokenizer.from_pretrained(HF_MODEL_ID)
    ...
    if feature_label == "engagement":
        capture_embedding_similarity(text_embedder, train_set, "./embed_checkpoints/", "train_xlnet_{}_embeddings.hdf5".format(addendum_label), device)
        capture_embedding_similarity(text_embedder, test_set, "./embed_checkpoints/", "test_xlnet_{}_embeddings.hdf5".format(addendum_label), device)
    elif feature_label == "sentiment":
        capture_embedding_similarity(text_embedder, train_set, "./embed_checkpoints/", "train_senti_xlnet_{}_embeddings.hdf5".format(addendum_label), device)
        capture_embedding_similarity(text_embedder, test_set, "./embed_checkpoints/", "test_senti_xlnet_{}_embeddings.hdf5".format(addendum_label), device)    
...
if __name__ == '__main__':
   ...
   # print("Running script with the following arguments: ", script_kwargs)
    ...
    default_alice_kwargs = {"model_output_dir": "/data1/s1930443/hf_models/",
                        ...
                        "HF_MODEL_ID": 'xlnet-large-cased',
                        #"HF_MODEL_ID": 'xlnet-large-cased',
                        "subtexts_aggregation_strategy": "mean",  
                        "pretrain_text_embedder": None, #"CosSim", #None, #"SimCE","TSDAE", "CosSim", "SimCE-Head-Body", 
                        "target_type": "mean_score", # "mean_score", "score_distribution
