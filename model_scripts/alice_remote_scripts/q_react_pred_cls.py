...
class CustomModelForArticleEmbedding(nn.Module):
    ...
    def forward(self, tokenized_input_subtexts, subtexts_attention_masks,targets=None, input_subtexts=None, return_attention_weights=False,**kwargs):
        ...
        subtext_embeddings = cls_pooling(self.embedder_base(input_ids=tokenized_input_subtexts.reshape(-1, tokenized_input_subtexts_shape[-1]), 
...
def main(model_output_dir, data_dirpath, HF_MODEL_ID, subtexts_aggregation_strategy, pretrain_subtext_embedder, target_type, deepspeed):
    ...
    if feature_label == "engagement":
        capture_embedding_similarity(text_embedder, train_set, dataset_inst.dynamic_batch_padding,"./embed_checkpoints/", "train_subtextLevel_xlnet_{}_embeddings.hdf5".format(addendum_label), device)
        capture_embedding_similarity(text_embedder, test_set,dataset_inst.dynamic_batch_padding ,"./embed_checkpoints/", "test_subtextLevel_xlnet_{}_embeddings.hdf5".format(addendum_label), device)
    elif feature_label == "sentiment":
        capture_embedding_similarity(text_embedder, train_set,dataset_inst.dynamic_batch_padding, "./embed_checkpoints/", "train_senti_subtextLevel_xlnet_{}_embeddings.hdf5".format(addendum_label), device)
        capture_embedding_similarity(text_embedder, test_set,dataset_inst.dynamic_batch_padding, "./embed_checkpoints/", "test_senti_subtextLevel_xlnet_{}_embeddings.hdf5".format(addendum_label), device)    
...
if __name__ == '__main__':
   ...
   # print("Running script with the following arguments: ", script_kwargs)
    ...
    default_alice_kwargs = {"model_output_dir": "/data1/s1930443/hf_models/",
                        ...
                        "HF_MODEL_ID": 'xlnet-large-cased', #'sentence-transformers/all-mpnet-base-v2', # 'sentence-transformers/all-distilroberta-v1' , # 'sentence-transformers/all-distilroberta-v1', #'sentence-transformers/all-mpnet-base-v2' ,
                        "subtexts_aggregation_strategy": "mean",  
