...
class CustomModelForArticleEmbedding(nn.Module):
    ...
    def forward(self, tokenized_input_subtexts, subtexts_attention_masks,targets=None, input_subtexts=None, return_attention_weights=False, subtext_checkpointing=True, **kwargs):
        ...
        if subtext_checkpointing:

            print("text subtext embed size ", text_subtext_embeddings.shape)
            print("text embed size ", text_embeddings.shape)
            print("subtext embed attn mask size ", subtext_embeddings_attention_masks.shape)

            if self.subtext_pooling_mode == "attention" and return_attention_weights:
                return (text_subtext_embeddings.cpu().detach().numpy(), subtext_embeddings_attention_masks.cpu().detach().numpy(), nn.functional.normalize(text_embeddings, p=2, dim=1), attention_weights).cpu().detach().numpy()
            elif self.subtext_pooling_mode == "gated" and return_attention_weights:
                return (text_subtext_embeddings.cpu().detach().numpy(), subtext_embeddings_attention_masks.cpu().detach().numpy(), nn.functional.normalize(text_embeddings, p=2, dim=1), gate_weights).cpu().detach().numpy()
            else:
                ...
                return text_subtext_embeddings.cpu().detach().numpy(), subtext_embeddings_attention_masks.cpu().detach().numpy(), text_embeddings.cpu().detach().numpy()
...
def main(model_output_dir, data_dirpath, HF_MODEL_ID, subtexts_aggregation_strategy, pretrain_subtext_embedder, target_type, deepspeed):
    ...
    # Checkpointing of embeddings:
    if subtexts_aggregation_strategy == "mean":

        article_id_list = []
        targets_list = []
        text_subtext_embeddings_list, subtext_embeddings_attention_masks_list, text_embedding_list = [], [], []
        temp_loader = DataLoader(train_set, batch_size=8, collate_fn=dataset_inst.dynamic_batch_padding,shuffle=True, drop_last=True)
    
        for temp_batch in tqdm(temp_loader):

            text_subtext_embeddings, subtext_embeddings_attention_masks, text_embeddings = text_embedder(temp_batch['tokenized_input_subtexts'].to(device), 
                                                                                                        temp_batch['subtexts_attention_masks'].to(device),
                                                                                                        return_attention_weights=False, 
                                                                                                        subtext_checkpointing=True,
                                                                                                        )
            

            text_subtext_embeddings_list.append(text_subtext_embeddings.cpu().detach().numpy())
            subtext_embeddings_attention_masks_list.append(subtext_embeddings_attention_masks.cpu().detach().numpy())
            text_embedding_list.append(text_embeddings.cpu().detach().numpy())

            article_id_list.extend(temp_batch['article_ids'])
            targets_list.append(temp_batch['targets'].cpu().numpy())
        
        #Pad text_subtext_embeddings with 3 dim to same shape along dimension 1.
        max_num_of_sentences = max([text_subtext_embeddings.shape[1] for text_subtext_embeddings in text_subtext_embeddings_list])
        text_subtext_embeddings_list = [np.pad(text_subtext_embeddings, ((0,0), (0,max_num_of_sentences - text_subtext_embeddings.shape[1]), (0,0)), mode='constant', constant_values=0) for text_subtext_embeddings in text_subtext_embeddings_list]
        subtext_embeddings_attention_masks_list = [np.pad(subtext_embeddings_attention_masks, ((0,0), (0,max_num_of_sentences - subtext_embeddings_attention_masks.shape[1])), mode='constant', constant_values=0) for subtext_embeddings_attention_masks in subtext_embeddings_attention_masks_list]

        text_subtext_embeddings = np.concatenate(text_subtext_embeddings_list, axis=0)
        del text_subtext_embeddings_list
        subtext_embeddings_attention_masks = np.concatenate(subtext_embeddings_attention_masks_list, axis=0)
        del subtext_embeddings_attention_masks_list
        text_embeddings = np.concatenate(text_embedding_list, axis=0)
        del text_embedding_list
        targets = np.concatenate(targets_list, axis=0)
        del targets_list

        with h5py.File("./embed_checkpoints/"+"mpnet_zero_embeddings.hdf5", 'w') as hf:
            hf.create_dataset("article_ids", data=article_id_list)
            hf.create_dataset("text_subtext_embeddings", data=text_subtext_embeddings)
            hf.create_dataset("subtext_embeddings_attention_masks", data=subtext_embeddings_attention_masks)
            hf.create_dataset("text_embeddings", data=text_embeddings)
            hf.create_dataset("targets", data=targets)
    else:
        text_subtext_embeddings, subtext_embeddings_attention_masks, text_embeddings, attention_weights = text_embedder()
